<template>
  <div class="h-[calc(100vh-theme('spacing.14'))] flex flex-col relative overflow-hidden">
    ...
    <div class="px-8 py-6 border-b border-white/5 bg-black/20 flex flex-col md:flex-row items-center justify-between gap-4">
       <div>
          ...
          <p class="text-sm text-gray-400 mt-1">Step-by-step file transfer assistance</p>
       </div>
       ...
       <div class="flex items-center gap-2">
          <div class="flex flex-col items-center gap-1 cursor-pointer" @click="canGoToStep(1) && (step = 1)">
             <div class="w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold border transition-all"
                :class="step >= 1 ? 'bg-[var(--win-accent)] text-black border-[var(--win-accent)] shadow-[0_0_10px_rgba(96,205,255,0.4)]' : 'bg-white/10 text-gray-500 border-white/10'"
             >1</div>
             <span class="text-[10px] font-medium uppercase tracking-wider" :class="step >= 1 ? 'text-[var(--win-accent)]' : 'text-gray-600'">Source</span>
          </div>
          <div class="w-12 h-0.5" :class="step >= 2 ? 'bg-[var(--win-accent)]/50' : 'bg-white/10'"></div>
          <div class="flex flex-col items-center gap-1 cursor-pointer" @click="canGoToStep(2) && (step = 2)">
             <div class="w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold border transition-all"
                :class="step >= 2 ? 'bg-[var(--win-accent)] text-black border-[var(--win-accent)] shadow-[0_0_10px_rgba(96,205,255,0.4)]' : 'bg-white/10 text-gray-500 border-white/10'"
             >2</div>
             <span class="text-[10px] font-medium uppercase tracking-wider" :class="step >= 2 ? 'text-[var(--win-accent)]' : 'text-gray-600'">Dest</span>
          </div>
          <div class="w-12 h-0.5" :class="step >= 3 ? 'bg-[var(--win-accent)]/50' : 'bg-white/10'"></div>
          <div class="flex flex-col items-center gap-1">
             <div class="w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold border transition-all"
                :class="step >= 3 ? 'bg-[var(--win-accent)] text-black border-[var(--win-accent)] shadow-[0_0_10px_rgba(96,205,255,0.4)]' : 'bg-white/10 text-gray-500 border-white/10'"
             >3</div>
             <span class="text-[10px] font-medium uppercase tracking-wider" :class="step >= 3 ? 'text-[var(--win-accent)]' : 'text-gray-600'">Review</span>
          </div>
       </div>
    </div>
    ...
    <div v-show="step === 1" class="flex-1 flex flex-col min-h-0 animate-in slide-in-from-right-10 fade-in duration-300">
       ...
       <!-- Footer Action -->
       <div class="p-6 border-t border-white/10 flex justify-between items-center bg-black/40">
          <div class="text-sm text-gray-400">
             Selected: <span class="text-white font-bold">{{ sourceSelection.size }}</span> items
          </div>
          <button 
             @click="step = 2" 
    ...
    <div v-show="step === 2" class="flex-1 flex flex-col min-h-0 animate-in slide-in-from-right-10 fade-in duration-300">
       <div class="px-8 py-3 bg-emerald-500/5 border-b border-emerald-500/10 text-emerald-200 text-sm flex items-center gap-2">
       ...
       <div class="p-6 border-t border-white/10 flex justify-between items-center bg-black/40">
          ...
          <div class="text-center text-sm text-gray-400">
              Current Path: <span class="text-white font-mono bg-white/10 px-2 py-1 rounded-lg ml-1">{{ destinationPath }}</span>
          </div>
          <button 
             @click="goToReview" 
             class="btn-primary flex items-center gap-2 px-8"
    ...
    <div v-if="step === 3" class="flex-1 flex flex-col items-center justify-center p-8 animate-in zoom-in-95 fade-in duration-300 relative">
       ...
       <div class="absolute w-[500px] h-[500px] bg-emerald-500/10 blur-[120px] rounded-full pointer-events-none -bottom-20 -right-20"></div>

       <!-- Manifest Card -->
       <div class="glass-panel w-full max-w-6xl relative overflow-hidden shadow-2xl">
           <!-- Gradient Border Line Top -->
           <div class="h-1 w-full bg-gradient-to-r from-[var(--win-accent)] via-white/20 to-emerald-500 opacity-50"></div>
           ...
           <div class="flex flex-col md:flex-row relative z-10 max-h-[70vh] md:max-h-none overflow-y-auto md:overflow-visible">
               ...
               <div class="flex-1 p-6 md:p-8 border-b md:border-b-0 md:border-r border-white/5 bg-gradient-to-b from-[var(--win-accent)]/5 to-transparent relative">
                   ...
                   <div class="flex items-center justify-between mb-6">
                       ...
                       <div class="text-xs font-mono text-gray-500 bg-white/5 px-2 py-1 rounded-lg">
                   ...
                   <div class="space-y-2 max-h-[200px] md:max-h-[300px] overflow-y-auto pr-2 custom-scrollbar mask-gradient-bottom">
                       ...
                       >
                           <UIcon name="i-heroicons-document" class="w-4 h-4 text-gray-500 group-hover:text-[var(--win-accent)] transition-colors" />
                           <span class="text-sm text-gray-300 font-mono truncate w-full group-hover:text-white transition-colors">
                               {{ item.split('/').pop() }}
                           </span>
                       </div>
                       <div v-if="sourceSelection.size > 50" class="text-center py-2 text-xs text-gray-500 font-mono italic">
               ...
               <div class="hidden md:flex w-16 relative items-center justify-center pointer-events-none">
                    ...
                    <div class="w-12 h-12 rounded-2xl bg-black/80 border border-white/10 flex items-center justify-center z-10 shadow-[0_0_30px_rgba(0,0,0,0.5)]">
               ...
               <!-- DESTINATION COLUMN -->
               <div class="flex-1 p-6 md:p-8 bg-gradient-to-b from-emerald-900/5 to-transparent relative flex flex-col">
                   <div class="flex items-center justify-between mb-6">
                        <div class="text-xs text-gray-500 font-mono">Target</div>
                        <div class="text-xs font-bold text-emerald-400 uppercase tracking-widest flex items-center gap-2">
                           Destination <div class="w-2 h-2 rounded-full bg-emerald-400 shadow-[0_0_10px_rgba(52,211,153,0.8)]"></div>
                       </div>
                   </div>
                   ...
                   <div class="flex-1 flex flex-col items-center justify-center py-4 md:py-8">
                       ...
                       <div class="relative group cursor-default">
                           <div class="absolute inset-0 bg-emerald-500/20 blur-2xl rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
                           <div class="w-24 h-24 md:w-32 md:h-32 rounded-3xl bg-gradient-to-br from-white/5 to-black border border-white/10 flex flex-col items-center justify-center shadow-2xl relative z-10 group-hover:border-emerald-500/30 transition-all duration-300 group-hover:-translate-y-1">
                               <UIcon name="i-heroicons-folder" class="w-10 h-10 md:w-12 md:h-12 text-emerald-400 mb-2" />
                               <div class="text-[10px] text-gray-500 uppercase tracking-wider">Drive</div>
                           </div>
                       </div>

                       <div class="mt-6 md:mt-8 w-full">
                           <div class="text-xs text-center text-gray-500 uppercase tracking-widest mb-2">Writing To</div>
                           <div class="bg-black/40 border border-white/10 rounded-xl p-4 flex items-center justify-center gap-3 group hover:border-emerald-500/20 transition-colors">
                               <span class="font-mono text-xs md:text-sm text-emerald-200 truncate max-w-[250px] md:max-w-none">{{ destinationPath }}</span>
                           </div>
                       </div>
                   </div>
               </div>
