<template>
  <div class="flex flex-col gap-6">
    <!-- Main Navigation -->
    <div class="p-1.5 bg-black/40 rounded-xl border border-white/10 flex gap-1">
      <button 
        v-for="type in types" 
        :key="type.value"
        ...
        :class="currentType === type.value 
          ? 'bg-[var(--brand-1)] text-black shadow-[0_0_15px_rgba(96,205,255,0.3)]' 
    ...
    <!-- Filters Section -->
    <div class="space-y-4">
      <h3 class="text-xs font-bold text-gray-500 uppercase tracking-widest px-1">Sort & Filter</h3>
      
      <!-- Sort By -->
      <div class="space-y-2">
        <label class="text-xs font-semibold text-gray-400 px-1">Sort By</label>
        <div class="relative group">
           <select 
             :value="currentSort"
             @input="$emit('update:sort', ($event.target as HTMLSelectElement).value)"
             class="w-full appearance-none bg-black/20 border border-white/10 rounded-xl px-4 py-3 text-sm text-white outline-none focus:border-[var(--brand-1)]/50 transition-colors cursor-pointer"
           >
           ...
           <UIcon name="i-heroicons-chevron-down" class="absolute right-4 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-500 pointer-events-none" />
        </div>
      </div>
